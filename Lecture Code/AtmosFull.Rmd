---
title: "Ozone concentration and temperature"
author: "Mr. Example"
...
plot(ozone ~ temp, data = means, type = "n")
mapply(function(x, y) {
  points(ozone ~ temp, data = subset(means, locale == x), col = y)
  mod <- lm(ozone ~ temp, data = subset(means, locale == x))
  abline(mod, col = y)
}, unique(means$locale), 1:4)
```
